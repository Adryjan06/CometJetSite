document.addEventListener("DOMContentLoaded", function () {
    ...
    // Animacje GSAP dla strony głównej
    gsap.to("nav a", {
        opacity: 1,
        y: 0,
        duration: 0.6,
    ...
    // Animacje dla strony Before Flight
    ...
    // Sprawdzenie, czy obrazy się ładują
...
const inneButton = document.getElementById('inne-button');
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Funkcja do ukrywania intro z animacją i usunięciem
    function hideIntro() {
      ...
      gsap.to(intro, {
        ...
        onComplete: () => {
          intro.remove();
          // Po usunięciu intro przywracamy scroll
    ...
    // Gdy wideo się kończy - ukryj intro
    if (video) {
      video.addEventListener('ended', hideIntro);
    }
  
    // Obsługa kliknięcia przycisku "Pomiń"
    if (skipButton) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (passwordSubmit && passwordInput && passwordScreen && gallery && errorMsg) {
      ...
      // Obsługa wciśnięcia Enter w polu input
